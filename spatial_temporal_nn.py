...
class VideoLevelEvaluation(Callback):
    def __init__(self, data_path, frame_counts_path, 
                 validation_data, interval=10, num_videos_eval=4, 
        ...
        self.video_paths = [os.path.join(data_path, filename) for filename in video_filenames] 
    ...
    def on_epoch_end(self, epoch, logs={}):
        if epoch % self.interval == 0:
            ...
            avg_eval_video_preds = np.zeros(shape=(self.num_videos_eval, self.num_classes))
            ...
            for _ in range(self.num_segments):
                ...
                eval_video_preds = self.model.predict(eval_video_frames, verbose=0)
                avg_eval_video_preds += eval_video_preds/self.num_segments # average the probabilities across segments
            ...
            print("\n Epoch: {:d} - eval. loss: {:.4f} - eval. acc.: {:.4f}".format(epoch+1, eval_loss, eval_acc))
