...
valid_video_paths = [
    os.path.join(
        'data/NewVideos/validation_videos/',
        video_name) for video_name in valid_video_names]
valid_video_frame_counts = [len(sorted(os.listdir(video_path)))
                            for video_path in valid_video_paths]

valid_video_frame_count = {}
for video_name, video_frame_count in zip(
        valid_video_names, valid_video_frame_counts):
